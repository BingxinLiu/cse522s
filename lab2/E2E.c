...
static int calibrate(void *data) {
	...
	while((*cur)->core == core) {
		...
		printk(KERN_INFO "CALIBRATE: task %u, subtask %u, exe_time %ums, loop count %u, core %u, prio %u\n", (*cur)->task_id, (*cur)->subtask_id, (*cur)->exe_time, (*cur)->loop_count, (*cur)->core, (*cur)->priority);
		cur++;
	}
    return 0;
}
...
static int E2E_init(void) {
	...
	if(strcmp(mode, "calibrate") == 0) {
		...
		for(i = 0; i < subtask_tot; i++) {
			...
			if(sub_ptr[i]->core == CORE_NUM) invalid = 1, sub_ptr[i]->core = 0;
		}
		sort(sub_ptr, subtask_tot, sizeof(struct subtask *), &comp_rel_ddl, NULL);
		for(i = 0; i < subtask_tot; i++) {
			printk("core %u %u\n", sub_ptr[i]->core, &sub_ptr[i]);
